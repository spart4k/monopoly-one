...
onMounted(() => {
  ...
  // 🔄 Авто-возврат в комнату
  const savedRoom = localStorage.getItem('monopoly_room_id')
  if (savedRoom && username.value.trim()) {
    sendEvent({ type: 'JOIN_ROOM', roomId: savedRoom, playerId: myId.value, name: username.value })
  }
...
const joinRoom = (roomId: string) => {
  if (!username.value.trim()) { alert('Введите имя'); return }
...
<!-- 📝 Template оставляем без изменений (только добавь :style="{ backgroundColor: getPlayerColorHex(slot.color) }" в слоты) -->
<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white flex flex-col">
    ...
    <main class="flex-1 p-6 grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-7xl mx-auto w-full">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div v-for="room in rooms" :key="room.id" class="bg-gray-800 rounded-xl p-4 border border-gray-700 hover:border-blue-500/50 transition flex flex-col">
            <div class="flex justify-between items-start mb-3">
              ...
              <span class="text-xs px-2 py-0.5 rounded bg-green-900/40 text-green-300 border border-green-800/50">{{ room.status === 'PLAYING' ? '🎮 В игре' : '⏳ Ожидание' }}</span>
            </div>
            <div class="grid grid-cols-2 gap-2 mb-4">
              <div v-for="(slot, i) in fillSlots(room.players, room.maxPlayers)" :key="i" class="p-3 rounded bg-gray-900/60 text-center border border-gray-700/50 flex flex-col items-center justify-center gap-2" :class="slot ? 'border-blue-500/30 bg-blue-900/20' : 'border-dashed'">
                <div v-if="slot" class="w-10 h-10 rounded-full border-2 border-white/90 shadow-md flex items-center justify-center text-lg font-bold text-white" :style="{ backgroundColor: getPlayerColorHex(slot.color) }">{{ slot.name?.charAt(0).toUpperCase() }}</div>
                ...
                <span v-if="slot" class="text-xs px-1.5 rounded" :class="slot.isReady ? 'bg-green-900/50 text-green-300' : 'bg-yellow-900/50 text-yellow-300'">{{ slot.isReady ? '✅ Готов' : '⏳ Ожидание' }}</span>
              </div>
            </div>
            <div class="text-xs text-gray-400 mb-3 flex justify-between"><span>💰 1500₽</span><span>👥 {{ room.players.length }}/4</span></div>
            <div class="mt-auto">
              <template v-if="isHost(room) && room.status === 'LOBBY'">
                <div class="relative group">
                  <button @click="startGame(room.id)" :disabled="!allReady(room)" class="w-full py-2.5 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 disabled:from-gray-600 disabled:to-gray-700 disabled:cursor-not-allowed text-white rounded-lg font-semibold transition shadow-lg">🚀 Начать игру</button>
                  <div v-if="!allReady(room)" class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-1.5 bg-gray-900 text-gray-200 text-xs rounded shadow-lg opacity-0 group-hover:opacity-100 transition pointer-events-none whitespace-nowrap border border-gray-700 z-10">⚠️ Дождитесь готовности всех игроков</div>
                </div>
              </template>
              <template v-else-if="isInRoom(room) && room.status === 'LOBBY'">
                <button @click="toggleReady(room.id)" class="w-full py-2.5 rounded-lg font-semibold transition border" :class="myStatus(room)?.isReady ? 'bg-green-600/20 text-green-400 border-green-500/50' : 'bg-yellow-600/20 text-yellow-400 border-yellow-500/50 hover:bg-yellow-600/30'">{{ myStatus(room)?.isReady ? '✅ Готов' : '⏳ Готов' }}</button>
              </template>
              <template v-else-if="room.status === 'LOBBY'"><button @click="joinRoom(room.id)" class="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-semibold transition shadow-lg">👋 Присоединиться</button></template>
      ...
      <div class="bg-gray-800 rounded-xl p-6 border border-gray-700 h-fit sticky top-6">
        ...
        <div class="space-y-4">
          <div><label class="block text-gray-300 text-sm mb-1.5">Название комнаты</label><input v-model="targetRoom" class="w-full px-3 py-2 bg-gray-900 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Room123" /></div>
          <button @click="targetRoom.trim() ? joinRoom(targetRoom) : joinRoom('Room-' + Math.random().toString(36).slice(2,5))" class="w-full py-2.5 bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white rounded-lg font-semibold transition shadow-lg">{{ targetRoom ? '🔗 Войти' : '🆕 Создать случайную' }}</button>
        </div>
      </div>
    </main>
  </div>
